...
- inline scripts can also read the document/window alias surfaces (`document.documentElement`, `document.head`, `document.body`, `document.title`, `document.location`, `document.URL`, `document.documentURI`, `document.baseURI`, `document.origin`, `window.title`, `window.location`, `window.origin`, `Element.baseURI`, and `Element.origin`), but those bindings remain internal to the script runtime and are not part of the public `Harness` API
