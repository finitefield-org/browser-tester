...
- node wrappers expose `nodeName`, `nodeType`, `nodeValue`, `data`, `textContent`, `length`, and CharacterData editing methods (`splitText()`, `substringData()`, `appendData()`, `insertData()`, `deleteData()`, and `replaceData()`)
...
- `form.elements.namedItem(name)` can surface `RadioNodeList` objects for multi-match groups, `RadioNodeList.forEach(callback[, thisArg])` is available, and `RadioNodeList.value` is writable; assigning a matching radio value checks the first matching radio, and assigning a missing value clears the group
