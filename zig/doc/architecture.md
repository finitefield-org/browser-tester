...
`CSSContainerRule.conditionText` / `CSSContainerRule.cssText` are writable on `@container` rules and rewrite the owning block in place while preserving nested rules, and `CSSSupportsRule.conditionText` / `CSSSupportsRule.cssText` are writable on `@supports` rules and rewrite the owning block in place while preserving nested rules.
