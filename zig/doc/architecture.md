...
- `document.location` and `window.location` as limited `Location` host objects (`href`, `hash`, `protocol`, `host`, `hostname`, `port`, `username`, `password`, `pathname`, `search`, `assign()`, `replace()`, `reload()`, `toString()`, and `valueOf()`), plus deterministic `hashchange` events, deterministic `popstate` events on history traversal, `window.onhashchange`, `window.onpopstate`, `window.onfocus`, `window.onblur`, `window.onpagehide`, `window.onpageshow`, `window.onscroll`, and `document.onscroll`
