...
`HTMLAnchorElement.download` / `HTMLAnchorElement.target` are reflected string attributes that feed the deterministic download/open click slice.
...
The stylesheet owner element slice also includes reflected `type`, `hreflang`, `charset`, `imageSrcset`, `imageSizes`, `fetchPriority`, and `crossOrigin` on `HTMLLinkElement`, and anchor target reflection feeds the deterministic open click slice.
