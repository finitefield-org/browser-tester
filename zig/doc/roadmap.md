...
`HTMLDataElement.value` and `HTMLTimeElement.dateTime` are also available as modern machine-readable content reflections on `data` and `time` elements; `time.dateTime` falls back to text content when `datetime` is absent.
