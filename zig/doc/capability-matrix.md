...
| Script DOM collection support | Stable Core | 7 | Available | Inline scripts and listeners can use `document.querySelectorAll(...)` and `element.querySelectorAll(...)`; results are minimal `NodeList` snapshots with `length` and `item(index)`. |
...
| Script HTML serialization surfaces | Stable Core | 8 | Available | Inline scripts can use `innerHTML`, `outerHTML`, and `insertAdjacentHTML` on `Element`; getters serialize the implemented subset deterministically, setters reuse bounded fragment parsing, position-aware insertion is supported, raw-text contexts stay raw, and malformed or lossy serialization fails explicitly. |
