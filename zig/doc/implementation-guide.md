...
The document/window alias slice now also includes `Node.contains(...)`, `Node.compareDocumentPosition(...)`, `Node.isSameNode(...)`, `Node.isEqualNode(...)`, `Node.nodeValue`, `Node.data`, `document.onreadystatechange`, `window.onpagehide`, `window.onpageshow`, and the minimal `window.navigator.languages` `DOMStringList`-like surface with `keys()`, `values()`, and `entries()`, plus the deterministic legacy aliases `userLanguage`, `browserLanguage`, `systemLanguage`, and `oscpu`.
