...
impl Harness {
    ...
    fn hoist_var_declarations(&mut self, stmts: &[Stmt], env: &mut HashMap<String, Value>) {
    ...
    fn mark_tdz_initialized(&mut self, pending_tdz_bindings: &mut HashSet<String>, name: &str) {
    ...
    fn await_value_in_for_await(&mut self, value: Value) -> Result<Value> {
                ...
                for stmt in stmts {
                    ...
                    match stmt {
                        ...
                        Stmt::ClassDecl {
                            ...
                            let (super_constructor, super_prototype) =
                                if let Some(super_class_expr) = super_class {
                                    ...
                                    if !Self::is_valid_extends_constructor_candidate(
                                        &evaluated_super,
                                    ) {
                                        return Err(Error::ScriptRuntime(
                                    ...
                                    if matches!(evaluated_super, Value::Null) {
                                        ...
                                        let super_prototype = self.object_property_from_value(
                                            &evaluated_super,
                                            "prototype",
                                        )?;
                                        if !matches!(
                            ...
                            for method in methods.iter().filter(|method| method.is_private) {
                                match method.kind {
                                    ...
                                    ClassMethodKind::Getter | ClassMethodKind::Setter => {
                                        if let Some(binding) =
                                            private_bindings.get_mut(&method.name)
                                        {
                            ...
                            for (field_index, field) in fields.iter().enumerate() {
                                let initializer = if field.is_private {
                                    ...
                                    let field_name =
                                        if let Some(name_expr) = field.computed_name.as_ref() {
                                            let key_value =
                                ...
                                if field.is_static {
                                    static_field_initializers_by_index
                                        .insert(field_index, initializer);
                                } else {
                                ...
                                match entry {
                                    ClassStaticInitializerDecl::Field(field_index) => {
                                        let initializer = static_field_initializers_by_index
                                            ...
                                            .ok_or_else(|| {
                                                Error::ScriptRuntime(
                                                    "class static field initializer is missing"
                                                        .into(),
                                                )
                                            })?;
                                        ...
                                        match initializer {
                                            ...
                                            RuntimeStaticInitializer::Block(handler) => {
                                                ...
                                                for sync_name in &outer_sync_names {
                                                    ...
                                                    if let Some(next) =
                                                        block_env.get(sync_name).cloned()
                                                    {
                                                        static_env.insert(
                                                            sync_name.clone(),
                                                            next.clone(),
                            ...
                            match prop {
                                ...
                                DomProp::Popover => {
                                    if self.node_explicit_own_property_overrides_dom_property(
                                        ...
                                        self.set_node_assignment_property(
                                            node, "popover", value, event, false,
                                        )?;
                                    } else {
                                ...
                                DomProp::TabIndex => {
                                    if self.node_explicit_own_property_overrides_dom_property(
                                        ...
                                        self.set_node_assignment_property(
                                            node, "tabIndex", value, event, false,
                                        )?;
                                    } else {
                                ...
                                DomProp::AudioAutoplay => {
                                    if self.node_explicit_own_property_overrides_dom_property(
                                        ...
                                        self.set_node_assignment_property(
                                            node, "autoplay", value, event, false,
                                        )?;
                                    } else {
                                        self.set_reflected_boolean_attribute(
                                            node,
                                ...
                                DomProp::AudioControls => {
                                    if self.node_explicit_own_property_overrides_dom_property(
                                        ...
                                        self.set_node_assignment_property(
                                            node, "controls", value, event, false,
                                        )?;
                                    } else {
                                        self.set_reflected_boolean_attribute(
                                            node,
                                ...
                                DomProp::AnchorDownload => {
                                    if self.node_explicit_own_property_overrides_dom_property(
                                        ...
                                        self.set_node_assignment_property(
                                            node, "download", value, event, false,
                                        )?;
                                    } else {
                                ...
                                DomProp::AnchorHreflang => {
                                    if self.node_explicit_own_property_overrides_dom_property(
                                        ...
                                        self.set_node_assignment_property(
                                            node, "hreflang", value, event, false,
                                        )?;
                                    } else {
                                ...
                                DomProp::AnchorCharset => {
                                    if self.node_explicit_own_property_overrides_dom_property(
                                        ...
                                        self.set_node_assignment_property(
                                            node, "charset", value, event, false,
                                        )?;
                                    } else {
                        ...
                        Stmt::NodeTreeMutation {
                            ...
                            match method {
                                NodeTreeMethod::After => {
                                    self.dom.insert_after(target_node, child)?
                                }
                                NodeTreeMethod::Append => {
                        ...
                        Stmt::ForOf {
                            ...
                            let for_of_result = (|| -> Result<ExecFlow> {
                                ...
                                let source = match iterable {
                                    ...
                                    Value::Set(set) => {
                                        ForOfSource::Values(set.borrow().values.clone())
                                    }
                                    Value::Object(entries) => {
                                        ...
                                        } else if let Some(iterator) = self
                                            .for_of_symbol_iterator_factory_result(
                                                &entries, event,
                                            )?
                                        {
                                ...
                                let loop_result = (|| -> Result<ExecFlow> {
                                    match source {
                                        ...
                                        ForOfSource::InternalIterator(iterator) => {
                                            loop {
                                                let Some(item) = self
                                                    .iterator_next_value_from_object(&iterator)?
                                                else {
                                                    break;
                                                };
                                                env.insert(item_var.clone(), item.clone());
                                    ...
                                    Value::Object(entries) => {
                                        if Self::is_async_iterator_object(&entries.borrow()) {
                                            ...
                                            while let Some(value) = self
                                                .async_iterator_next_value_from_object(&entries)?
                                            {
                                                out.push(value);
                                            }
                                            out
                                            ...
                                            if let Some(factory) = async_iterator_factory {
                                                ...
                                                let iterator_value = self.execute_callable_value(
                                                    &factory,
                                                    &[],
                                                    event,
                                                )?;
                        ...
                        Stmt::Switch { expr, clauses } => {
                            ...
                            let previous =
                                self.collect_direct_block_lexical_bindings(&all_clause_stmts, env);
