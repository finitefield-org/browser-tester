...
impl Harness {
    ...
    pub(crate) fn try_eval_event_and_delete_expr(
        &mut self,
        expr: &Expr,
        env: &HashMap<String, Value>,
            ...
            match expr {
                            ...
                            let value = match prop {
                                ...
                                EventExprProp::TargetId => {
                                    if let Some(value) = self.live_event_nested_property_from_env(
                                        ...
                                        Value::String(
                                            self.dom.attr(event.target, "id").unwrap_or_default(),
                                        )
                                    }
                                }
                                EventExprProp::CurrentTargetId => {
                ...
                Expr::Delete(inner) => match inner.as_ref() {
                    ...
                    Expr::ObjectPathGet { target, path } => {
                        ...
                        let final_key = path.last().ok_or_else(|| {
                            Error::ScriptRuntime("object path cannot be empty".into())
                        })?;
                    ...
                    Expr::DomRead { target, prop } => {
                        if let Some(value) =
                            self.delete_non_node_dom_prop_fallback(target, prop, env)?
                        {
                            Ok(value)
                        } else {
