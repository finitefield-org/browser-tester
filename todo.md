...
The remaining `tests/integration_cases/*.rs` files not listed below are a 1:1 filename/function match between `tests/` and `next/crates/browser-tester/tests/`.
...
Status: `partial`
Note: next keeps the same scenario under `click_handler_observes_updated_let_capture_for_clipboard`; the async clipboard version remains only in root.
...
### Next-only additions
The following `next/` contract test files do not have root equivalents yet: `contract_dom_phase1.rs`, `contract_harness_builder.rs`, `contract_phase3.rs`, `contract_phase4.rs`, `contract_phase6.rs`, `contract_phase7.rs`, `contract_phase8.rs`, and `contract_script_phase2.rs`.
