...
#[test]
fn type_text_updates_value_and_dispatches_input_listener() -> browser_tester_next::Result<()> {
...
#[test]
fn click_toggles_checkbox_and_dispatches_input_listener() -> browser_tester_next::Result<()> {
    let mut harness = Harness::from_html(
...
#[test]
fn click_submit_button_triggers_form_submit_default_action() -> browser_tester_next::Result<()> {
    let mut harness = Harness::from_html(
...
#[test]
fn submit_dispatches_form_submit_listener_directly() -> browser_tester_next::Result<()> {
    let mut harness = Harness::from_html(
        "<form id='profile'><input id='name'><button id='submit' type='submit'>Save</button></form><div id='out'></div><script>document.getElementById('profile').addEventListener('submit', () => { document.getElementById('out').textContent = document.getElementById('name').value; });</script>",
    )?;
...
#[test]
fn dispatch_runs_target_listener_without_default_action() -> browser_tester_next::Result<()> {
    let mut harness = Harness::from_html(
...
#[test]
fn type_text_on_non_form_control_reports_a_dom_error() -> browser_tester_next::Result<()> {
