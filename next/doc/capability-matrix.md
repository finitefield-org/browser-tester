...
| Typed mock registry families | Stable Test Mocks | 0 | Available | Fetch, dialogs, clipboard, location, file input, downloads, and storage seeds are modeled, with public mock actions wired for the non-download families. |
...
| Clipboard, dialog, location, and file-input capture | Stable Test Mocks | 4 | Available | Public actions are wired and each family exposes deterministic capture through the registry. |
| Download artifact capture | Stable Test Mocks | 4 | Skeleton | Artifact capture exists in the registry, but no public action is wired yet. |
