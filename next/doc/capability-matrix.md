...
| Simple pseudo-classes (`:first-child`, `:last-child`, `:checked`, `:disabled`, `:enabled`) | Stable Core | post-7 | Available | The bounded selector engine now handles the common structural and form-state pseudo-classes, while `:nth-child`, `:not`, `:is`, and broader CSS parsing stay out of scope. |
...
| Script DOM query hardening (broad pseudo-classes deferred) | Stable Core | 7 | Available | Unsupported selector syntax remains explicit, selector lists are supported, and broader CSS parsing stays out of scope. |
