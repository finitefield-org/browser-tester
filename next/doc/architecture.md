...
- `Element.matches`
- `Element.closest`
- no `querySelectorAll` or collection types in this phase
...
Phase 6 selector expansion is complete; slices 1 through 4 for class selectors, compound simple selectors, descendant combinators, child combinators, and selector hardening are delivered. A backlog slice adds adjacent sibling combinators (`A + B`) through the same bounded selector engine, and any future selector work should follow the post-Phase-6 backlog-driven slice mode.
Phase 7 script DOM query expansion is complete for slices 1 through 4; `document.querySelector`, `element.querySelector`, `Element.matches`, `Element.closest`, and selector hardening are implemented, while `querySelectorAll` and collection types remain out of scope.
