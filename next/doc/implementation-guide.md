...
It also includes `document.location`, `window.location`, `document.URL`, `document.documentURI`, `document.baseURI`, `element.baseURI`, `document.origin`, `window.origin`, `element.origin`, `document.currentScript`, and `document.readyState` during inline script bootstrap through the runtime location mock and session bootstrap state.
