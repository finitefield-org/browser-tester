...
The same reflected option surface also includes `option.disabled` as the disabled-attribute alias used by the same state and selector surfaces, and the same disabled state is now reflected on common form controls via `input.disabled` / `textarea.disabled` / `button.disabled` / `select.disabled`.
...
The same reflected form-control surface also includes `min` / `max` as the reflected string range bounds on `input` elements, and those bounds feed the bounded `:in-range` / `:out-of-range` selector checks for number controls.
