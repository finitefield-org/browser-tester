...
It also includes `document.location`, `window.location`, `document.URL`, `document.documentURI`, `document.baseURI`, `element.baseURI`, `document.origin`, `document.referrer`, `window.name`, `window.origin`, `element.origin`, `window.localStorage`, `window.sessionStorage`, `document.currentScript`, `document.readyState`, `document.compatMode`, `document.characterSet`, `document.charset`, `document.contentType`, `document.activeElement`, and `window.children` as an alias of `document.children` during inline script bootstrap through the runtime location mock, storage surface, focus state, session bootstrap state, and document metadata surface.
...
- dialogs, clipboard, print, close, and open are simpler than fetch/navigation
