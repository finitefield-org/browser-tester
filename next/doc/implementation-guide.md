...
The same reflected owner-form surface also includes `input.form` / `button.form` / `select.form` / `textarea.form` / `option.form` / `fieldset.form` / `output.form` / `object.form` / `embed.form` as the owning form element, and it updates when the control is moved out of its form; the current owner lookup follows ancestor forms only.
