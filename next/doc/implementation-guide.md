...
The same reflected option surface also includes `option.disabled` as the disabled-attribute alias used by the same state and selector surfaces.
...
The same reflected option surface also includes `option.text` as the text-content alias used by the same option text surface.
...
The same reflected select surface also includes `select.selectedIndex` as the active option index for that same selected state.
...
The same reflected option surface also includes `option.form` as the owning form element, and it updates when the option is moved out of its form.
