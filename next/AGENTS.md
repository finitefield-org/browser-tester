...
この `next/` ディレクトリは、既存 `browser-tester` の延長実装ではなく、`next.md` を元にした作り直し用 workspace である。

既存実装を参照してもよいが、設計判断は `next/` 配下の文書を正とすること。

## 最初に参照する文書

`next/` で作業を始める前に、少なくとも次を確認すること。

1. `next/README.md`
2. `next/doc/implementation-guide.md`
3. `next/doc/subsystem-map.md`
4. `next/doc/capability-matrix.md`
5. `next/doc/roadmap.md`

必要に応じて追加で参照する文書:

- `next/doc/architecture.md`
- `next/doc/mock-guide.md`
- `next/doc/limitations.md`
- `next/doc/adr/*.md`
...
公開 API や内部コードの置き場は、必ず `next/doc/subsystem-map.md` を見て決めること。
...
日々の実装順は `next/doc/implementation-guide.md` を基準にすること。
...
- `next/doc/capability-matrix.md` を更新したか
- `next/README.md` を更新したか
...
- `next/README.md` の更新
- `next/doc/mock-guide.md` の更新
- `next/doc/capability-matrix.md` の更新
...
- support level や公開保証が変わったら `next/doc/capability-matrix.md`
- mock の入口や capture が増えたら `next/doc/mock-guide.md`
- 実装順や進め方の判断が変わったら `next/doc/implementation-guide.md`
- 所有権や置き場の方針が変わったら `next/doc/subsystem-map.md`
- 利用者向け入口が変わったら `next/README.md`

## この workspace 固有の注意

- `next/` は clean-room rewrite のための場所であり、既存 `src/` の構造に引きずられないこと。
- ただし、既存実装とテストから得られた知見は再利用してよい。
