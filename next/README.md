...
- Phase 7 script DOM query slices 1 through 4, selector lists, simple pseudo-classes, `document.querySelector(...)`, `element.querySelector(...)`, `Element.matches(...)`, `Element.closest(...)`, and selector hardening are implemented; `querySelectorAll` with minimal `NodeList` support, `Element.children` with minimal `HTMLCollection` support, `getElementsByTagName(...)` with live `HTMLCollection` support, `getElementsByClassName(...)` with live `HTMLCollection` support, and `getElementsByName(...)` with live `NodeList` support are implemented, including `namedItem()` on HTMLCollection, while broader collection APIs remain deferred
