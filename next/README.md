...
- Phase 7 script DOM query slices 1 through 4, selector lists, simple pseudo-classes, `document.querySelector(...)`, `element.querySelector(...)`, `Element.matches(...)`, `Element.closest(...)`, and selector hardening are implemented; `querySelectorAll` with minimal `NodeList` support, `Element.children` with minimal `HTMLCollection` support, `getElementsByTagName(...)` with live `HTMLCollection` support, `getElementsByClassName(...)` with live `HTMLCollection` support, `getElementsByName(...)` with live `NodeList` support, `document.forms` / `form.elements` live `HTMLCollection` support, `select.options` live `HTMLCollection` support, and `document.images` / `document.links` live `HTMLCollection` support are implemented, including `namedItem()` on HTMLCollection, while broader collection APIs remain deferred
