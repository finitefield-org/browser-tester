...
- backlog-driven work now focuses on additional specialized live collections beyond `document.childNodes`, `document.styleSheets`, `document.children`, `table.rows` / `tr.cells`, `select.selectedOptions`, `fieldset.elements`, `datalist.options`, `map.areas`, `table.tBodies`, and `element.labels`, selector grammar broadening, and HTML serialization broadening
