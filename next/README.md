...
- The live `HTMLCollection` surfaces used for `document.images`, `document.links`, `document.embeds`, `document.plugins`, `document.anchors`, `document.applets`, and `document.all` also expose `keys()`, `values()`, `entries()`, and `forEach()` iterator helpers.
