...
- The live `HTMLCollection` surfaces used for `document.children`, `window.children`, `document.images`, `document.links`, `document.embeds`, `document.plugins`, `document.anchors`, `document.applets`, `document.scripts`, and `document.all` also expose `keys()`, `values()`, `entries()`, and `forEach()` iterator helpers.
...
- HTML serialization broadening slice 1 `insertAdjacentHTML`, slice 2 `insertAdjacentElement()` / `insertAdjacentText()`, slice 3 `template.content.innerHTML`, slice 4 namespace-aware serialization compatibility, and slice 5 `document.open()` / `document.write()` / `document.writeln()` / `document.close()` are implemented with bounded fragment insertion / serialization on `<template>` content, adjusted SVG / MathML name handling, browser-style escaping for mixed-quote attribute values, basic character reference decoding during fragment parse, including common named references such as `&nbsp;`, append-style document write helpers that append to the open document tree, and chainable `document.open()` / `document.close()` returns
