...
- `querySelectorAll` と minimal `NodeList` は既に実装済みで、`NodeList.forEach` / `HTMLCollection.forEach` と `NodeList.keys()` / `NodeList.values()` / `NodeList.entries()` / `HTMLCollection.keys()` / `HTMLCollection.values()` / `HTMLCollection.entries()`、`document.scripts` も実装済み、selector lists と bounded pseudo-classes も実装済み、`Element.children` / minimal `HTMLCollection`（`namedItem()` 含む）と `document.childNodes` / live `NodeList`、`template.content` / live fragment-like collections と `template.content.innerHTML` / `template.content.textContent` / `template.content.querySelector(All)`、`document.children` / live `HTMLCollection`、`table.rows` / `tbody.rows` / `thead.rows` / `tfoot.rows` / `tr.cells` / live `HTMLCollection`、`getElementsByTagName` / `getElementsByTagNameNS` / `getElementsByClassName` / live `HTMLCollection`、`getElementsByName` / live `NodeList`、`document.forms` / `form.elements` / live `HTMLCollection`（多重一致時は `RadioNodeList`、そのグループでは `entries()` と writable `value` も利用可能）、`select.options` / `select.selectedOptions` / live `HTMLCollection`（`select.options.add()` / `select.options.remove()` も利用可能）、`document.documentElement` / `document.head` / `document.body` / `document.scrollingElement` / `document.title` / `window.title`、`document.images` / `document.links` / `document.embeds` / `document.plugins` / `document.anchors` / `document.applets` / `document.styleSheets`（`keys()` / `values()` / `entries()` / `namedItem()` / `forEach()` も利用可能）/ `document.all` / live `HTMLCollection`、`element.labels` on labelable form controls / fieldset / live `NodeList` support、`classList.value` / `classList.toString()` / `classList.item()` / `classList.forEach()` / `classList.keys()` / `classList.values()` / `classList.entries()` も実装済み、`Element.attributes` / `Attr` / `NamedNodeMap`（`setNamedItem()` / `setNamedItemNS()` / `removeNamedItem()` / `removeNamedItemNS()` / `forEach()` を含む）や collection-like `toString()` helpers も実装済み
