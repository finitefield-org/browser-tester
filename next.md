...
- `window.open()` / `window.close()` / `window.print()` / `window.scrollTo()` / `window.scrollBy()` / `window.scrollX` / `window.scrollY` / `window.pageXOffset` / `window.pageYOffset` / `window.navigator` (`userAgent`, `platform`, `language`, `cookieEnabled`, `onLine`) / `window.devicePixelRatio` / `document.visibilityState` / `document.hidden` / `document.hasFocus()` や download の観測
- `eval` 非対応による制御容易性
