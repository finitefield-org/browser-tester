...
- backlog slices は次の 3 つに整理する
  ...
  - selector grammar broadening: `:scope`, `:has(...)`, structural selector expansion, `:lang(...)`, `:dir(...)`, `:link`, `:any-link`, `:placeholder-shown`, `:required`, `:optional`, `:focus`, `:focus-within`, `:defined`, and bounded `:nth-child(... of <selector-list>)` / `:nth-last-child(... of <selector-list>)` / `:nth-of-type(... of <selector-list>)` / `:nth-last-of-type(... of <selector-list>)` are delivered; broader CSS parsing beyond the bounded selector engine remains deferred until needed by a specific user-visible gap
