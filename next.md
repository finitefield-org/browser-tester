...
- detached HTML / namespace-aware element / text node / comment / fragment construction 用の `document.createElement()` と `document.createElementNS()` と `document.createTextNode()` と `document.createComment()` と `document.createDocumentFragment()` に加えて、`before()` と `after()` と `cloneNode()` と `remove()` と `removeChild()` と `normalize()` と `importNode()` と `replaceWith()` も script host から使える。`template.content` の `cloneNode()` と `textContent` も入った。`Element.click()` / `Element.focus()` / `Element.blur()` も script から使える
...
- fake clock による時間制御
- `Math.random()` の固定化
- `fetch` / `clipboard` / `location` / `file input` / `matchMedia` / `open` / `close` / `print` / `scroll` のモック注入と、`navigator.clipboard.writeText()` / `navigator.clipboard.readText()` の deterministic な compatibility shim
- `window.open()` / `window.close()` / `window.print()` / `window.scrollTo()` / `window.scrollBy()` / `window.scrollX` / `window.scrollY` / `window.pageXOffset` / `window.pageYOffset` / `window.navigator` (`userAgent`, `appCodeName`, `appName`, `appVersion`, `product`, `productSub`, `vendor`, `vendorSub`, `pdfViewerEnabled`, `doNotTrack`, `javaEnabled()`, `plugins`, `mimeTypes`, `platform`, `language`, `userLanguage`, `browserLanguage`, `systemLanguage`, `oscpu`, `cookieEnabled`, `onLine`, `webdriver`, `hardwareConcurrency`, `maxTouchPoints`, `keys()`, `values()`, `entries()`, `forEach()` on `navigator.languages` / `navigator.mimeTypes`, plus deterministic no-op `refresh()` on `navigator.plugins`) / `window.devicePixelRatio` / `window.innerWidth` / `window.innerHeight` / `window.outerWidth` / `window.outerHeight` / `window.screenX` / `window.screenY` / `window.screenLeft` / `window.screenTop` / `window.screen` (`availWidth`, `availHeight`, `availLeft`, `availTop`, `colorDepth`, `pixelDepth`, `orientation.type`, `orientation.angle`) / `document.cookie` / `document.domain` / `window.self` / `window.window` / `window.parent` / `window.top` / `window.closed` / `window.opener` / `window.frames` / `window.length` / `form.length` / `select.length` / `window.history.length` / `window.history.state` / `window.history.scrollRestoration` / `window.history.pushState()` / `window.history.replaceState()` / `document.visibilityState` / `document.hidden` / `document.scrollingElement` / `document.hasFocus()` や download の観測
- `element.translate` も reflected element surface に入り、祖先の `translate` 状態を継承する
- `element.role` も reflected element surface に入り、plain attribute reflection として扱う
- `element.ariaLabel` も reflected element surface に入り、plain attribute reflection として扱う
- `element.ariaDescription` も reflected element surface に入り、plain attribute reflection として扱う
- `element.ariaRoleDescription` も reflected element surface に入り、plain attribute reflection として扱う
- `element.ariaHidden` も reflected element surface に入り、plain attribute reflection として扱う
- `element.tabIndex` も reflected element surface に入り、bounded integer reflection として扱う
- `eval` 非対応による制御容易性
...
- `:nth-last-child(even)`
- `:nth-last-child(an+b)`
- `:nth-of-type(<positive integer>)`
- `:nth-of-type(odd)`
- `:nth-of-type(even)`
- `:nth-of-type(an+b)`
- `:nth-last-of-type(<positive integer>)`
...
- `:nth-of-type(... of <selector-list>)`
- `:nth-last-of-type(... of <selector-list>)`
- `:is(...)`
- `:not(...)`
- `:scope`
- `:has(...)`
...
- Stable Test Mocks:
  ...
  - open mock
  - print mock
  - location mock / URL stringification helpers
  - file input mock
  - matchMedia mock
- Experimental Browser Facades:
...
- 追加の selector / collection backlog slice として `querySelectorAll` と minimal `NodeList`、selector lists、bounded pseudo-classes（`not(<compound selector>)`, `is(<compound selector>, ...)`, `where(<compound selector>, ...)`, `lang(<language-range>)`, `dir(<ltr|rtl>)`, `link`, `any-link`, `placeholder-shown`, `required`, `optional`, `focus`, `focus-visible`, `focus-within`, `target`, `root`, `empty`, `only-child`, `only-of-type`, `first-of-type`, `last-of-type`, `nth-child(<positive integer>)`, `nth-child(odd)`, `nth-child(even)`, `nth-child(an+b)`, `nth-last-child(<positive integer>)`, `nth-last-child(odd)`, `nth-last-child(even)`, `nth-last-child(an+b)`, `nth-of-type(<positive integer>)`, `nth-of-type(odd)`, `nth-of-type(even)`, `nth-of-type(an+b)`, `nth-last-of-type(<positive integer>)`, `nth-last-of-type(odd)`, `nth-last-of-type(even)`, `nth-last-of-type(an+b)` を含む）、`Element.children` / minimal `HTMLCollection`（`namedItem()` 含む）と `document.childNodes` / live `NodeList`、`template.content` / live fragment-like collections と `template.content.getElementById()` / `template.content.querySelector(All)`、`document.children` / `window.children` / live `HTMLCollection`、`table.rows` / `tbody.rows` / `thead.rows` / `tfoot.rows` / `tr.cells` / live `HTMLCollection`、`getElementsByTagName` / live `HTMLCollection`、`getElementsByTagNameNS` / live `HTMLCollection`、`getElementsByClassName` / live `HTMLCollection`、`getElementsByName` / live `NodeList`、`document.forms` / `form.elements` / live `HTMLCollection`、`select.options` / `select.selectedOptions` / live `HTMLCollection`、`document.documentElement` / `document.head` / `document.body` / `document.scrollingElement` / `document.title` / `window.title` / `document.location` / `window.location` / `document.URL` / `document.documentURI` / `document.baseURI` / `element.baseURI` / `Node.ownerDocument` / `Element.ownerDocument` / `Node.parentNode` / `Element.parentNode` / `Element.parentElement` / `Node.firstChild` / `Element.firstChild` / `Document.firstChild` / `Node.lastChild` / `Element.lastChild` / `Document.lastChild` / `Node.nextSibling` / `Element.nextSibling` / `Document.nextSibling` / `Node.previousSibling` / `Element.previousSibling` / `Document.previousSibling` / `firstElementChild` / `lastElementChild` / `childElementCount` reflection helpers / `document.origin` / `document.referrer` / `document.cookie` / `window.name` / `window.self` / `window.window` / `window.parent` / `window.top` / `window.origin` / `element.origin` / `window.localStorage` / `window.sessionStorage` / `document.currentScript` / `document.readyState` / `document.compatMode` / `document.characterSet` / `document.charset` / `document.contentType` / `document.visibilityState` / `document.hidden` / `document.activeElement` / `document.hasFocus()` / `window.children` / `window.frames` / `window.length` / `window.screen` (`availWidth`, `availHeight`, `availLeft`, `availTop`, `colorDepth`, `pixelDepth`, `orientation.type`, `orientation.angle`) during inline script bootstrap、`document.images` / `document.links` / `document.embeds` / `document.plugins` / `document.anchors` / `document.scripts` / `document.styleSheets` / `document.all` / live `HTMLCollection`、`form.noValidate` / `input.formNoValidate` / `button.formNoValidate` / `select.type` / `input.type` / `button.type` / `input.form` / `button.form` / `select.form` / `textarea.form` / `option.form` / `fieldset.form` / `output.form` / `object.form` / `embed.form` も実装済み
- さらに reflected element surface には `element.spellcheck` と `element.inputMode` も含まれる
- `input.indeterminate` も reflected form-control surface に含まれ、checkbox-only の indeterminate state を `:indeterminate` と同期する
- `input.defaultChecked` も reflected form-control surface に含まれ、checkbox / radio の default-checked state を `:default` と同期する
- `input.accept` / `input.multiple` も reflected form-control surface に含まれ、file-input configuration を同じ attribute store と同期する
- `form.action` / `form.method` / `form.enctype` / `form.target` と `input.formAction` / `button.formAction` / `input.formMethod` / `button.formMethod` / `input.formEnctype` / `button.formEnctype` / `input.formTarget` / `button.formTarget` / `optgroup.disabled` / `optgroup.label` / `fieldset.disabled` / `input.step` / `input.size` / `textarea.rows` / `textarea.cols` / `textarea.wrap` も reflected form-control surface に含まれ、共有 attribute store と同期し、`action` は `document.baseURI` に解決される
- Phase 8 は DOM mutation と reflection expansion を対象にしていたが、slice 1 から slice 5 まで完了済みで、tree mutation primitives には `removeChild()` と `replaceWith()` も含まれ、`Node.contains()` / `Element.contains()` / `Document.contains()` / `Node.compareDocumentPosition()` / `Element.compareDocumentPosition()` / `Document.compareDocumentPosition()` / `Node.hasChildNodes()` / `Element.hasChildNodes()` / `Document.hasChildNodes()` / `Node.firstChild` / `Element.firstChild` / `Document.firstChild` / `Node.lastChild` / `Element.lastChild` / `Document.lastChild` / `Node.nextSibling` / `Element.nextSibling` / `Document.nextSibling` / `Node.previousSibling` / `Element.previousSibling` / `Document.previousSibling` / `Node.nextElementSibling` / `Element.nextElementSibling` / `Node.previousElementSibling` / `Element.previousElementSibling` も同じ reflection surface に入った。さらに `form.noValidate` / `input.formNoValidate` / `button.formNoValidate` / `form.method` / `form.enctype` / `form.target` / `input.formMethod` / `button.formMethod` / `input.formEnctype` / `button.formEnctype` / `input.formTarget` / `button.formTarget` / `optgroup.disabled` / `optgroup.label` / `fieldset.disabled` も同じ form-control reflection surface に加わった。`element.accessKey` / `element.slot` / `element.autocapitalize` / `element.dir` / `element.lang` / `element.title` / `element.ariaLabel` / `element.ariaHidden` / `element.hidden` も同じ reflected element surface にある
- `element.ariaDescription` / `element.ariaRoleDescription` も同じ reflected element surface にある
- `element.translate` も同じ reflected element surface にあり、`translate='no'` を祖先から継承する
- 同じ reflection surface には `Node.isSameNode()` / `Element.isSameNode()` / `Document.isSameNode()` / `Node.isEqualNode()` / `Element.isEqualNode()` / `Document.isEqualNode()` も含まれる
...
7. bounded pseudo-classes (`:not(...)`, `:is(...)`, `:where(...)`, `:lang(...)`, `:dir(...)`, `:link`, `:any-link`, `:placeholder-shown`, `:required`, `:optional`, `:focus`, `:focus-visible`, `:focus-within`, `:defined`, `:first-child`, `:last-child`, `:root`, `:empty`, `:only-child`, `:only-of-type`, `:first-of-type`, `:last-of-type`, `:nth-child(<positive integer>)`, `:nth-child(odd)`, `:nth-child(even)`, `:nth-child(an+b)`, `:nth-last-child(<positive integer>)`, `:nth-last-child(odd)`, `:nth-last-child(even)`, `:nth-last-child(an+b)`, `:nth-of-type(<positive integer>)`, `:nth-of-type(odd)`, `:nth-of-type(even)`, `:nth-of-type(an+b)`, `:nth-of-type(... of <selector-list>)`, `:nth-last-of-type(<positive integer>)`, `:nth-last-of-type(odd)`, `:nth-last-of-type(even)`, `:nth-last-of-type(an+b)`, `:nth-last-of-type(... of <selector-list>)`, `:checked`, `:disabled`, `:enabled`, `:default`, `:valid`, `:invalid`, `:in-range`, `:out-of-range`, `:read-only`, `:read-write`)（post-Phase-7 selector slice, 完了）
   - `:focus-visible` is also implemented and follows the same focus state as `:focus`
   - `:indeterminate` is also implemented
   - bounded logical, structural, language, and simple form-state pseudo-classes resolve through the same bounded selector engine
...
8. `Element.children` and minimal `HTMLCollection`（post-Phase-7 collection slice, 完了）
   ...
   - `length` と `item(index)`
   - `namedItem()` は実装済み
   - broader collection APIs は deferred
9. `getElementsByTagName` and live `HTMLCollection`（post-Phase-7 collection slice, 完了）
   - document / element scoped live descendant tag-name collection
   - `length` と `item(index)`
   - `namedItem()` は実装済み
10. `getElementsByClassName` and `getElementsByName`（post-Phase-7 collection slice, 完了）
...
11. `document.forms` and `form.elements`（post-Phase-7 collection slice, 完了）
   ...
   - elements collections は `length` と `item(index)` と `namedItem(name)`、多重一致時は `RadioNodeList`、そのグループでは `keys()` / `values()` / `entries()` / `forEach()` と writable `value` も使える
...
12b. `select.selectedOptions`（post-Phase-7 collection slice, 完了）
   - select-scoped live selected-option HTMLCollection
   - `length` と `item(index)` と `namedItem(name)`
13. `document.images` / `document.links` / `document.embeds` / `document.plugins` / `document.anchors`（post-Phase-7 collection slice, 完了）
...
14. `document.scripts`（post-Phase-7 collection slice, 完了）
   ...
   - `length` と `item(index)` と `namedItem(name)`
14b. `document.styleSheets`（post-Phase-7 collection slice, 完了）
   - document-scoped live StyleSheetList
   - `length` と `item(index)` と `namedItem(name)`
14c. `document.childNodes`（post-Phase-7 collection slice, 完了）
   - document-scoped live NodeList
   - `length` と `item(index)`
15. `document.all`（post-Phase-7 collection slice, 完了）
   - document-scoped live all-elements HTMLCollection
   - `length` と `item(index)` と `namedItem(name)`
16. `getElementsByTagNameNS`（post-Phase-7 collection slice, 完了）
   - document / element scoped live namespace-aware descendant HTMLCollection
   - HTML / SVG / MathML namespace URI と `*` に限定
   - `length` と `item(index)` と `namedItem(name)`
...
1. attribute reflection (implemented)
   ...
   - `Element.attributes` / `Attr` / `NamedNodeMap`
   - `Attr.specified`
   - `Attr.isId`
   ...
   - `id`, `class`, `name`, `checked`, `disabled`, `indeterminate`, `selected`, `value`, `step` reflection
...
- docs, contract tests, and regression tests agree
- bounded HTML serialization surfaces, including `document.open()` / `document.write()` / `document.writeln()` / `document.close()`, round-trip the implemented subset; `document.open()` / `document.close()` return `document`, and `document.write()` appends into the open document tree after the current root is cleared
...
- `querySelectorAll` と minimal `NodeList` は既に実装済みで、`NodeList.forEach` / `HTMLCollection.forEach` と `NodeList.keys()` / `NodeList.values()` / `NodeList.entries()` / `HTMLCollection.keys()` / `HTMLCollection.values()` / `HTMLCollection.entries()`、`document.scripts` も実装済み、selector lists と bounded pseudo-classes も実装済み、`Element.children` / minimal `HTMLCollection`（`namedItem()` 含む）と `document.childNodes` / live `NodeList`、`template.content` / live fragment-like collections と `template.content.innerHTML` / `template.content.textContent` / `template.content.querySelector(All)`、`document.children` / live `HTMLCollection`、`table.rows` / `tbody.rows` / `thead.rows` / `tfoot.rows` / `tr.cells` / live `HTMLCollection`、`getElementsByTagName` / `getElementsByTagNameNS` / `getElementsByClassName` / live `HTMLCollection`、`getElementsByName` / live `NodeList`、`document.forms` / `form.elements` / live `HTMLCollection`（多重一致時は `RadioNodeList`、そのグループでは `keys()` / `values()` / `entries()` / `forEach()` と writable `value` も利用可能）、`select.options` / `select.selectedOptions` / live `HTMLCollection`（`select.options.add()` / `select.options.remove()` も利用可能）、`select.value` も実装済み、`document.documentElement` / `document.head` / `document.body` / `document.scrollingElement` / `document.title` / `window.title`、`document.images` / `document.links` / `document.embeds` / `document.plugins` / `document.anchors` / `document.applets` / `document.styleSheets`（`keys()` / `values()` / `entries()` / `namedItem()` / `forEach()` も利用可能）/ `document.all` / live `HTMLCollection`（iterator helpers も利用可能）、`element.labels` on labelable form controls / fieldset / live `NodeList` support、`classList.value` / `classList.toString()` / `classList.item()` / `classList.forEach()` / `classList.keys()` / `classList.values()` / `classList.entries()` も実装済み、`Element.attributes` / `Attr` / `NamedNodeMap`（`setNamedItem()` / `setNamedItemNS()` / `removeNamedItem()` / `removeNamedItemNS()` / `forEach()` を含む）や collection-like `toString()` helpers も実装済み
- backlog slices は次の 3 つに整理する
  - collection API broadening: slices 1 (`NodeList.forEach` / `HTMLCollection.forEach`), 2 (`document.scripts`), 3 (`document.anchors`), 4 (iterator-style helpers), 5 (`document.applets`), 6 (`document.children`), 7 (`table.rows` / `tbody.rows` / `thead.rows` / `tfoot.rows` / `tr.cells`), 8 (`select.selectedOptions`), 9 (`fieldset.elements` / `datalist.options` / `map.areas` / `table.tBodies`), 10 (`element.labels` on labelable form controls / fieldset), 11 (`document.styleSheets`), 12 (`document.childNodes`), 13 (`template.content.childNodes` / `template.content.children`), and 14 (`select.options.add()` / `select.options.remove()`) are delivered, collection-like `toString()` helpers are also available, and the remaining slices are additional specialized live collections beyond `document.styleSheets`, `document.children`, `template.content`, `template.content.querySelector(All)`, `table.rows` / `tr.cells`, `select.selectedOptions`, `fieldset.elements`, `datalist.options`, `map.areas`, `table.tBodies`, and `element.labels`; `form.elements.namedItem()` can surface `RadioNodeList` for multi-match groups and that `RadioNodeList` exposes writable `value`
  - selector grammar broadening: `:scope`, `:has(...)`, structural selector expansion, `:lang(...)`, `:dir(...)`, `:link`, `:any-link`, `:placeholder-shown`, `:blank`, `:required`, `:optional`, `:focus`, `:focus-within`, `:defined`, and bounded `:nth-child(... of <selector-list>)` / `:nth-last-child(... of <selector-list>)` / `:nth-of-type(... of <selector-list>)` / `:nth-last-of-type(... of <selector-list>)` are delivered; broader CSS parsing beyond the bounded selector engine remains deferred until needed by a specific user-visible gap
  - HTML serialization broadening: `insertAdjacentHTML` (delivered) -> `insertAdjacentElement()` / `insertAdjacentText()` (delivered) -> `template.content.innerHTML` / `DocumentFragment` serialization (delivered) -> namespace-aware serialization compatibility (delivered) -> browser-style mixed-quote attribute escaping and basic character reference decoding, including common named references such as `&nbsp;`, `&copy;`, and `&reg;` plus safe semicolonless forms like `&nbsp` / `&amp` / `&lt` / `&gt` / `&copy` / `&reg`, legacy uppercase variants like `&AMP` / `&LT` / `&GT` / `&QUOT` / `&NBSP` / `&COPY` / `&REG`, and semicolonless numeric forms like `&#160` / `&#xA0` (delivered) -> `document.open()` / `document.write()` / `document.writeln()` / `document.close()` (delivered)
- Phase 8 is complete in this workspace; slices 1 (attribute reflection), 2 (class / dataset views), 3 (tree mutation primitives including `before()` / `after()` / `removeChild()` / `replaceWith()`), 4 (HTML serialization surfaces), and 5 (mutation hardening and regression coverage) are already implemented, and the containment / child-presence reflection surface also includes `Node.hasChildNodes()` / `Element.hasChildNodes()` / `Document.hasChildNodes()`, plus reflected `input.autocomplete` / `textarea.autocomplete` / `input.placeholder` / `textarea.placeholder` / `input.defaultValue` / `textarea.defaultValue` / `input.autofocus` / `textarea.autofocus` / `button.autofocus` / `select.autofocus` / `input.min` / `input.max` / `input.indeterminate` / `input.defaultChecked` / `input.accept` / `input.multiple` / `select.type` / `input.type` / `button.type`, and `Element.attributes` / `Attr` / `NamedNodeMap` (including `NamedNodeMap.keys()` / `NamedNodeMap.values()` / `NamedNodeMap.entries()` / `NamedNodeMap.forEach()` and `Attr.specified` / `Attr.isId`)
- `Node.isConnected` / `Element.isConnected` / `Document.isConnected` and `Node.contains()` / `Element.contains()` / `Document.contains()` / `Node.compareDocumentPosition()` / `Element.compareDocumentPosition()` / `Document.compareDocumentPosition()` / `Node.hasChildNodes()` / `Element.hasChildNodes()` / `Document.hasChildNodes()` / `Node.firstChild` / `Element.firstChild` / `Document.firstChild` / `Node.lastChild` / `Element.lastChild` / `Document.lastChild` / `Node.nextSibling` / `Element.nextSibling` / `Document.nextSibling` / `Node.previousSibling` / `Element.previousSibling` / `Document.previousSibling` / `Node.nextElementSibling` / `Element.nextElementSibling` / `Node.previousElementSibling` / `Element.previousElementSibling` are also part of the current DOM reflection surface, with detached `template.content` staying disconnected, and reflected `input.disabled` / `textarea.disabled` / `button.disabled` / `select.disabled` / `fieldset.disabled` and `input.indeterminate` / `input.defaultChecked` / `input.accept` / `input.multiple` / `input.minLength` / `textarea.minLength` / `input.maxLength` / `textarea.maxLength` / `input.min` / `input.max` / `input.step` / `input.size` / `input.pattern` / `textarea.rows` / `textarea.cols` / `textarea.wrap` now feed the same bounded `:disabled` / `:enabled`, `:indeterminate`, `:default`, `:valid` / `:invalid`, `:in-range` / `:out-of-range`, and `:blank` selector state for form controls; `input.size` is also exposed as a reflected non-negative size attribute on `<input>` elements and defaults to `20` when absent, while `textarea.rows` / `textarea.cols` are reflected as non-negative textarea dimensions and default to `2` / `20` when absent; `textarea.wrap` is reflected as the textarea wrap mode and defaults to `soft` when absent
- The same DOM reflection surface also includes `Node.isSameNode()` / `Element.isSameNode()` / `Document.isSameNode()` and `Node.isEqualNode()` / `Element.isEqualNode()` / `Document.isEqualNode()`.
