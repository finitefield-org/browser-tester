...
- fake clock による時間制御
- `Math.random()` の固定化
- `fetch` / `clipboard` / `location` / `file input` / `matchMedia` / `open` / `close` / `print` のモック注入
- `window.open()` / `window.close()` / `window.print()` や download の観測
- `eval` 非対応による制御容易性
